...
DEFAULT_CONTAMINATION: float = 0.1       # Default contamination parameter for IsolationForest
...
DEFAULT_N_ESTIMATORS_LARGE: int = 25   # Default number of estimators for datasets > 500 rows
DEFAULT_N_ESTIMATORS_SMALL: int = 50   # Default number of estimators for datasets <= 500 rows
...
class AnomalyDetector:
    ...
    def __init__(self, db_path: str, max_cache_size: int = DEFAULT_MAX_CACHE_SIZE):
        ...
        self.cache_access_times: Dict[str, float] = {}    # Track when models were last accessed
        self.executor = ThreadPoolExecutor(max_workers=DEFAULT_THREAD_WORKERS)
        self.logger = logger.bind(component="AnomalyDetector")
        self.table_schemas: Dict[str, List[str]] = {}     # Maps table names to their column names
        self.numerical_columns_cache: Dict[str, List[str]] = {}  # Maps tables to numerical columns
        self.initialization_complete: bool = False
        self.background_tasks: List[asyncio.Task] = []
        self.training_in_progress: Set[str] = set()  # Track which models are currently training
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info("AnomalyDetector initialized successfully", db_path=self.db_path)
    ...
    def _start_background_model_training(self) -> None:
        ...
        available_priority_tables = [table for table in self.PRIORITY_TABLES 
        ...
        self.logger.info(f"Starting background model training for {len(available_priority_tables)} priority tables")
        
        # Create and start a background task for each priority table
        for table in available_priority_tables:
            if table in self.numerical_columns_cache and self.numerical_columns_cache[table]:
                columns = self.numerical_columns_cache[table]
                task = asyncio.create_task(
                    self._train_model_for_table(table, columns)
                )
    ...
    async def _train_model_for_table(self, table: str, columns: List[str]) -> None:
        ...
        try:
            ...
            df, sampling_applied = await self._get_table_data_with_sampling(table, columns, DEFAULT_MAX_ROWS)
            if df.empty:
            ...
            df_clean = df.dropna()
            if df_clean.empty:
                self.logger.warning(f"No clean data available for {table} after removing NaN values")
                return
            ...
            n_estimators = DEFAULT_N_ESTIMATORS_LARGE if len(df_subset) > 500 else DEFAULT_N_ESTIMATORS_SMALL
            ...
            with duckdb.connect(self.db_path, read_only=True) as conn:
                ...
                for table in tables:
                    try:
                        ...
                        self.logger.warning(f"Could not preload schema for table {table}", error=str(e))
    ...
    def _get_cached_model(self, table: str, columns: List[str]) -> Optional[Dict[str, Any]]:
    ...
    def _cache_model(self, table: str, columns: List[str], model_info: Dict[str, Any]) -> None:
    ...
    async def _train_model_in_background(self, model, df_subset, table, columns, top_columns, sampling_applied):
        ...
        try:
            ...
            self.logger.error(f"Error in background training for table {table}", error=str(e))
        finally:
            # Remove from in-progress set
            self.training_in_progress.discard(cache_key)
    ...
    def get_numerical_columns(self, table: str, requested_columns: List[str]) -> List[str]:
        ...
        if table in self.numerical_columns_cache:
            ...
            return [col for col in requested_columns if col in self.numerical_columns_cache[table]]
        ...
        try:
            ...
            # Remove duplicates
            self.numerical_columns_cache[table] = list(set(self.numerical_columns_cache[table]))
            
        except Exception as e:
            self.logger.error(f"Error determining numerical columns for table {table}", error=str(e))
    ...
    async def detect_anomalies(
        ...
        if table not in self.table_schemas:
            try:
                ...
                return {"status": "error", "message": f"Failed to access database: {str(e)}"}
        ...
        if cached_model_info:
            self.logger.info(f"Using cached model for table {table}", columns=numerical_columns)
            return await self._apply_cached_model(table, numerical_columns, cached_model_info, max_rows)
            
        # No cached model, create a new one
        return await self._create_and_apply_model(table, numerical_columns, max_rows, contamination, wait_for_model)
    ...
    async def _apply_cached_model(
        ...
        try:
            ...
            if not available_columns:
                return {"status": "error", "message": "None of the model's training columns are available in the current data"}
            ...
            self.logger.error(f"Error applying cached model for table {table}", error=str(e))
            return {"status": "error", "message": f"Failed to apply cached model: {str(e)}"}
    ...
    async def _create_and_apply_model(
        ...
        try:
            # Get data with sampling
            df, sampling_applied = await self._get_table_data_with_sampling(table, columns, max_rows)
            if df.empty:
                return {"status": "success", "data": [], "anomaly_count": 0, "row_count": 0}
            ...
            if len(df_clean.columns) > MAX_COLUMNS:
                self.logger.info(f"Limiting anomaly detection to {MAX_COLUMNS} columns out of {len(df_clean.columns)}")
            ...
            if not wait_for_model:
                ...
                task = asyncio.create_task(self._train_model_in_background(
                    model, df_subset, table, columns, top_columns, sampling_applied))
                self.background_tasks.append(task)  # Store the task to prevent garbage collection
                
                # Return a placeholder result
                self.logger.info(f"Model training started in background for table {table}")
                return {
                    "status": "success",
            ...
            # Process results
            return self._format_results(df_subset, predictions, anomaly_scores, sampling_applied, table=table)
    ...
    async def _get_table_data(self, table: str, columns: List[str], max_rows: int) -> pd.DataFrame:
        ...
        try:
            with duckdb.connect(self.db_path, read_only=True) as conn:
                ...
                if max_rows > 0 and row_count > max_rows:
                    ...
                    self.logger.info(f"Applying {sample_percent:.2f}% sampling to table {table}")
    ...
    async def detect_anomalies_batch(
        ...
        if not existing_tables:
            return {
                "status": "error",
                "message": "No valid tables found to analyze"
            }
        ...
        for table in existing_tables:
            ...
            if time.time() - start_time > time_limit_seconds:
                results["tables_skipped"].extend([t for t in existing_tables if t not in results["tables_processed"]])
                results["status"] = "partial_success"
                results["message"] = f"Time limit reached after processing {len(results['tables_processed'])} tables"
                break
    ...
    def _format_results(
        ...
        if len(results) > MAX_TOOL_OUTPUT_ROWS:
            ...
            return {
                ...
                "anomaly_count": sum(1 for r in results[:MAX_TOOL_OUTPUT_ROWS] if r["is_anomaly"]),
