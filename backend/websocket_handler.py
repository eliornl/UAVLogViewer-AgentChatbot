...
from backend.models import Session, Message, ChatRequest, ChatResponse, ResponseStatus, Metadata
...
async def handle_websocket_message(websocket: WebSocket, session_id: str, data: Dict[str, Any]) -> None:
    ...
    message_id = data.get('message_id', data.get('messageId', str(datetime.now(timezone.utc).timestamp())))
    ...
    async with per_session_locks[session_id]:
        ...
        if session.status != "READY":
            ...
            if session.status == "PENDING":
                ...
                error_message = session.status_message or "Session is in an error state."
        ...
        if session_id not in agents:
            try:
                ...
                from backend.main import initialize_agent, STORAGE_DIR, DUCKDB_FILE_PREFIX
                import os
                import time
